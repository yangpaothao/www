    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function submitFormannouncement(){
                if($("#txttitle").val() == ""){
                    alert("Please type a title for this announcement.");
                    $("#txttitle").focus();
                    return(false);
                }
                if($("#txtexpiredate").val() == ""){
                    alert("Please enter a expire date for this announcement.");
                    $("#txtexpiredate").focus();
                    return(false);
                }
                if($("#txtdate").val() == ""){
                    alert("Please type a message or data for this announcement.");
                    $("#txtdata").focus();
                    return(false);
                }  
                //We will check to see if the name exists in our database.
                var thisFrmdata = new FormData($('#frmAddannouncement')[0]);
                thisFrmdata.append('from', 'announcement');
                $.ajax({
                    type: 'POST',
            ...
            function showThisannouncement(recno, status='Readonly'){
                ...
                window.open('announcement.php?recno='+recno+'&status='+status, '_blank');
            }
        ...
        <table id="tbl_order_data" class="tbl-order-data">
            <thead>
                <tr style="background-color: #173346;" style="font-size: .8em;">
                    ...
                    <th style="width: 80px; position: sticky; top: 0px; z-index: 10; padding-right: 10px;" >Attachment1</th>
                    <th style="<?= $canmod ?>"></th>
                 </tr>
            </thead>
            <tbody><?php
                    ...
                    if($result)
                    {
                        ...
                        {?>
                            <tr id="tr<?=$i?>" style="font-size: .8em;">
                                ...
                                if(in_array('Modify', $_SESSION['thisauth']) ||  in_array('Delete', $_SESSION['thisauth']))
                                {?>
                                    <td style="height: 40px; width: 125px; padding-left: 10px;">
                ...
                <tr>
                    <td>
                        <div class="main-div-body-announcement-left" style="margin-top: -30px;">
                            <div class="main-div-body-announcement-header">Announcement</div>
