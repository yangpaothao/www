...
function Main()
{
    ...
    $thisarray = json_decode($_GET['thisarray']);
    foreach($thisarray as $rsa)
    {?>
        <div class="main-div" style="page-break-after: always;"><?php
            ...
            $sql .= "ON flow.recno = so.foreignkey_flow_recno LEFT JOIN signatures s ON so.recno = s.so_foreign_key WHERE so.recno=".$rsa." AND so.isdeleted = false";
            $result = $db->PDOMiniquery($sql);
            //file_put_contents("./dodebug/debug.txt", 'multiple prints: '.$sql, FILE_APPEND);?>        
                ...
                <div style="height: 700px; boarder: 1px solid black;">
                    <table style="font-size: .8em; background-color: white; boarder: none;"><?php
                        ...
                        {?>
                            ...
                            $sqlso .= "WHERE soi.isdeleted = false AND fk_service_orders = ".$rsa." ORDER BY c.name";
                                ...
                                foreach($sqlsoresult as $rsseciont)
                                {
                                    ...
                                    if($tempcat == "" || $tempcat != $curcat)
                                    {
                                       ...
                                       <tr class="tr-<?=$thiscrecno?>" id="tr_<?=$thiscrecno?>"><td class="tbl-order-lbl" style="width: 180px; text-align: left; font-size: 1em; font-weight: bold; height: 20px;" colspan="4">
                                               <div style="float: left; margin-right: 5px;" class="div-category-list-item" id="div_category_<?=$thiscrecno?>_<?=$rsa?>">
                                                    <?=$curcat?>
                                               </div>
                                           </td>
                                       </tr>
                                    ...
                                    <div id="div_<?=$thiscrecno?>_<?=$thisirecno?>" class="div_service-order-data-service-container">
                                        ...
                                        <div class="div-service-order-service-input-container">
                                            <input class="service-order-inputs service-order-inputs-item" type="text" id="txtitem_<?=$thisirecno?>_<?=$rsa?>" value="<?=$rsseciont['value'];?>" />
                                        </div>
                                    </div><?php
                                }?>
                                        </div> 
                                ...
                                if($db->PDORowcount($resultsection) > 0)
                                {
                                    ...
                                    foreach($resultsection as $rsseciont)
                                    {
                                        ...
                                        if($tempcat == "" || $tempcat != $curcat)
                                        {
                                           ...
                                           <tr class="tr-<?=$thiscrecno?>" id="tr_<?=$thiscrecno?>">
                                               <td class="tbl-order-lbl" style="width: 100%; text-align: left; font-size: 1em; font-weight: bold; height: 20px;" colspan="4">
                                                    <div style="float: left; margin-right: 5px;" class="div-category-list-item" id="div_category_<?=$curcat?>_<?=$rsa?>">
                                                        <?=$curcat?>
                                                    </div>
                                               </td>
                                           </tr>
                                        ...
                                        <div id="div_<?=$thiscrecno?>_<?=$thisirecno?>" class="div_service-order-data-service-container">
                                            ...
                                            <div class="div-service-order-service-input-container">
                                                <input class="service-order-inputs service-order-inputs-item" type="text" id="txtitem_<?=$thisirecno?>_<?=$rsa?>" value="" />
                                            </div>
                                        </div><?php
                                    }?>
                                            </div> 
                            ...
                            <td class="tbl-order-lbl" colspan="4"><?php
                                if(!is_null($rs['completedby']))
                                {
                                    ...
                                    //file_put_contents("./dodebug/debug.txt", $sqls, FILE_APPEND);?>
                                        ...
                                        $result = $db->PDOMiniquery($sqls); //Someone completed this SO# so we show all the people that worked on this SO
                                        foreach($result as $rss)
                                        {
                                            $sqlsignature = "SELECT * FROM signatures WHERE so_foreign_key = ".$rsa." AND em_foreign_key = ".$rss['recno'];
